...
for (const [routeName, permission] of Object.entries(mapping)) {
  const regex = new RegExp(`(name:\\s*['"]${routeName}['"],\\s*component:[^,]+,\\s*meta:\\s*\\{[\\s\\S]*?)(?!\\s*permission:)(,\\s*|\\n\\s*)(\\})`, 'g');
  // Only inject if permission is not already there
...
// Add for all routes that start with the routeName (e.g. operation_shipments_add)
for (const [routeName, permission] of Object.entries(mapping)) {
