          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              <h1 class="font-bold text-2xl leading-tight text-gray-900 uppercase">
                SRV #{{ requisition.referenceNumber || '-------' }}
              </h1>
        ...
        <div class="flex flex-row items-center justify-between md:justify-end gap-3 md:gap-4">
          ...
          <div class="flex gap-2">
            <Button
              ...
              variant="outline"
              size="md"
              @click="$router.push(`/inventory/store-requisition/edit/${requisitionId}`)"
            >
              Edit
            </Button>
            <Button
...
import { fetch_store_requisition_details } from "../../api/inventory.api";
...
const { data: response, isLoading } = useQuery({
  queryKey: ["store-requisition", requisitionId],
  queryFn: () => fetch_store_requisition_details(requisitionId),
...
const activeTabComponent = computed(() => {
  switch (activeTab.value) {
    case "overview":
      return StoreRequisitionOverviewTab;
    default:
      return StoreRequisitionOverviewTab;
  }
});
