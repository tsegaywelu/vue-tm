  ...
  <div class="flex flex-col gap-8 p-8 max-w-[1600px] mx-auto min-h-screen animate-fade-in">
    ...
    <div class="flex justify-between items-center">
      <div class="flex flex-col gap-2">
        <h1 class="text-3xl font-extrabold text-primary tracking-tight">Approval Requests</h1>
        <p class="text-gray-500 font-medium text-lg">Manage and process operational payment approvals from one central hub.</p>
      </div>
    </div>
...
import type { ApprovalFilterParams, ApprovalRequest, ApprovalAction } from '../operation.types'
...
const approvalMutation = useMutation({
  mutationFn: async ({ row, action }: { row: ApprovalRequest, action: ApprovalAction }) => {
    switch (row.payableType) {
      case 'advancePayment':
        return await update_advance_status(row.advancePaymentId || row._id, action)
      case 'transactions':
...
const handleApprovalAction = ({ row, action }: { row: ApprovalRequest, action: ApprovalAction }) => {
