<template>
  ...
  <Teleport to="#extra-page-data" defer v-if="currentTab === 'shipment' && tableRef?.summaryItems">
    <div v-if="!tableRef?.summaryItems" class="flex justify-center items-center py-2">
...
const handleGenerateInvoice = async () => {
  ...
  if (confirmed) {
    const totalAmount = selectedRows.value.reduce((sum, row) => sum + (row.totalPrice || 0), 0);
...
const handleExport = (type: 'raw' | 'full') => {
  ...
  if (type === 'raw') {
    filteredData = data.filter((s: any) => s.productType === 'IN_BOUND' || s.productType === 'SITE_TRANSFER');
  } else {
    filteredData = data.filter((s: any) => s.productType === 'OUT_BOUND');
  }

  if (filteredData.length === 0) {
    toast.warning(`No ${type === 'raw' ? 'Raw Material' : 'Full Product'} data found to export`);
    return;
  }
  ...
  XLSX.writeFile(workbook, `Receivable_${type === 'raw' ? 'Raw_Material' : 'Full_Product'}_${new Date().toISOString().split('T')[0]}.xlsx`);
};
</script>
