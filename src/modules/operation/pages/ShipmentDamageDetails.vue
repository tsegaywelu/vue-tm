      ...
      <div class="flex items-center gap-4 self-start md:self-auto">
        ...
        <div class="flex flex-col items-start md:items-end gap-1">
          ...
          <Status
            ...
            type="wrapped"
            :label="damage.receivableStatus || 'Unknown'"
          />
        </div>
      </div>
    </div>
...
import { fetch_shipment_damage_by_id } from "../api/operation.api";
import { currencyFormatter, dateFormatter } from "@/utils/utils";

// Tabs
import BasicDetailsTab from "../components/damage-details/BasicDetailsTab.vue";
import ItemsTab from "../components/damage-details/ItemsTab.vue";

const route = useRoute();
const damageId = route.params.id as string;

const { data: damageData, isLoading } = useQuery({
  queryKey: ["shipment-damage", damageId],
  queryFn: () => fetch_shipment_damage_by_id(damageId),
});
