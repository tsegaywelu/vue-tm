  ...
  <div class="flex flex-col gap-6">
    <div class="bg-grey-25 rounded-3xl p-6 border border-grey-100">
      <div class="flex items-center justify-between mb-6">
        <h3 class="text-lg font-bold text-grey-900">Vehicle Documents</h3>
      </div>

      <div v-if="vehicle.vehicleDocuments?.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <DocumentCard
          v-for="(doc, index) in vehicle.vehicleDocuments"
          :key="index"
          :title="getDocumentName(doc)"
          :document-type="'vehicle-document'"
          :file-path="doc"
