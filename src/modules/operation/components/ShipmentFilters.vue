<template>
  <div class="flex flex-wrap items-end gap-4 bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
    <!-- Faceted Search -->
    ...
    <div class="flex gap-4 min-w-[400px]">
      <div class="flex-1">
        <label class="block text-xs font-bold text-gray-400 uppercase mb-2 ml-1 text-grey-400">Origin</label>
        <SelectInput
      ...
      <div class="flex-1">
        <label class="block text-xs font-bold text-gray-400 uppercase mb-2 ml-1 text-grey-400">Destination</label>
        <SelectInput
    ...
    <div class="min-w-[180px]">
       <label class="block text-xs font-bold text-gray-400 uppercase mb-2 ml-1 text-grey-400">Status</label>
       <SelectInput
    ...
    <div class="relative min-w-[200px]">
      <label class="block text-xs font-bold text-gray-400 uppercase mb-2 ml-1 text-grey-400">Date Range</label>
      <button
        type="button"
      ...
      >
        ...
        <div class="size-4 opacity-40 transition-transform" :class="{ 'rotate-180': showDatePicker }" v-html="all_icons.leftAngle" style="transform: rotate(-90deg);"></div>
      </button>
      ...
      >
        ...
        <div class="flex flex-col gap-2 min-w-[140px] pt-2">
          <button
            ...
            :class="isShortcutActive(shortcut) ? 'bg-primary text-white shadow-md' : 'hover:bg-gray-50 text-gray-500'"
          >
            {{ shortcut.label }}
          </button>
        </div>
...
const { setSearch, state } = useTablePagination('shipment-list')
...
const filters = reactive<ShipmentFilterParams>({
  routeOrigin: '',
  routeDestination: '',
  status: '',
  startDate: '',
...
const handleDateSelect = (val: string | { start: string; end: string | null }) => {
...
watch(() => state.value.search, (new_val) => {
