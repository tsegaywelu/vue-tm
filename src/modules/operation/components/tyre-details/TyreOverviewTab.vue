      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <InfoWrapper title="General Information">
          <div class="flex flex-col gap-4">
            ...
            <TyreDataLabel label="Price" :value="tyre?.price ? currencyFormatter(tyre.price) : '-'" />
          </div>
        </InfoWrapper>

        <InfoWrapper title="Vehicle & Position">
          <div class="flex flex-col gap-4">
            <TyreDataLabel label="Vehicle Plate" :value="tyre?.vehicle?.plateNumber" />
            <TyreDataLabel label="Is Trailer" :value="tyre?.trailer ? 'Yes' : 'No'" />
            <TyreDataLabel label="Position" :value="formatStatus(tyre?.tyrePosition)" />
          </div>
        </InfoWrapper>
        ...
        <InfoWrapper title="Installation Information">
          <div class="flex flex-col gap-4">
            <TyreDataLabel label="Installation Date" :value="tyre?.installationDate ? dateFormatter(tyre.installationDate) : '-'" />
            <TyreDataLabel label="Installation Mileage" :value="tyre?.installationMileage" />
          </div>
        </InfoWrapper>
      </div>
    </div>
