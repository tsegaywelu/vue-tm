  ...
  <Form :id="formId" :values="initialValues" :onSubmit="handleSubmit">
    <template #default="{ form }">
      <Colapsable
        ...
        description="Define the role name, type, and specific permissions."
      >
        ...
        <div class="mt-10">
          <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex gap-2">
              <button
                type="button"
                @click="selectAllPermissions(form)"
              ...
              <button
                type="button"
                @click="clearAllPermissions(form)"
            ...
            >
              ...
              <div class="flex flex-wrap gap-2">
                ...
                >
                  <input
                    type="checkbox"
                    :checked="isPermissionChecked(form, subject, action)"
                    @change="(e) => togglePermission(form, subject, action, (e.target as HTMLInputElement).checked)"
...
const isPermissionChecked = (form: any, subject: string, action: string) => {
  const permissions = form.state.values.permissions || [];
  const perm = permissions.find((p: any) => p.subject === subject);
  return perm?.action?.includes(action) || false;
};

const togglePermission = (form: any, subject: string, action: string, checked: boolean) => {
  const permissions = [...(form.state.values.permissions || [])];
...
const selectAllPermissions = (form: any) => {
...
const clearAllPermissions = (form: any) => {
