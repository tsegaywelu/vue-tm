      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="sm:hidden flex flex-col divide-y divide-gray-100 rounded-xl border border-gray-200 bg-white overflow-hidden">
          ...
          <div v-for="(item, index) in requisition?.items" :key="index" class="px-4 py-3 grid grid-cols-6 gap-x-2 gap-y-1.5">
            ...
            <div class="col-span-2 text-right"><span class="text-sm font-bold text-primary">Qty: {{ item.quantity }}</span></div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50 text-white">
              <tr class="bg-primary/90 text-white">
                ...
                <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">SRV No</th>
                <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">Lead Time</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-for="(item, index) in requisition?.items" :key="index" class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-600">{{ requisition.srv?.referenceNumber || '-' }}</td>
                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-600">-</td>
