  ...
  <div class="bg-surface rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col">
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-2">
        <span class="mdi mdi-speedometer text-red-400 text-lg"></span>
        <h3 class="font-semibold text-gray-800">Speeding Alerts</h3>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-xs font-semibold bg-red-50 text-red-500 px-2 py-0.5 rounded-full">{{ rows.length }}</span>
...
const rows = computed(() => {
  ...
  const list: any[] = Array.isArray(raw) ? raw
    ...
    : [];
  return [...list]
    .filter((a: any) => a.alertType === "SPEED")
