<template>
  <div class="bg-surface rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col">
    <div class="flex items-center justify-between mb-4">
      <h3 class="font-semibold text-gray-800">Bolo Expiry</h3>
    ...
    <div v-else-if="!rows.length" class="text-sm text-gray-400 text-center py-8">
      No expiring bolos in {{ days }} days
    </div>
...
import { fetchBoloExpiresIn, fetchBoloExpired } from "../../api/dashboard.api";
...
const { data: expiresData, isLoading: loadingExpires } = useQuery({
  queryKey: ["dashboard-bolo-expires", days],
  queryFn: () => fetchBoloExpiresIn(days.value),
});

const { data: expiredData, isLoading: loadingExpired } = useQuery({
  queryKey: ["dashboard-bolo-expired"],
  queryFn: fetchBoloExpired,
});
...
const rows = computed(() => {
  ...
  return [...expired, ...expires]
    ...
    .map((v: any) => ({
      id: v._id,
      plate: v.plateNumber ?? "—",
      expiry: v.boloExpirationDate
        ? new Date(v.boloExpirationDate).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })
        : "—",
      expired: v.boloExpirationDate ? new Date(v.boloExpirationDate) < today : false,
    }));
});
</script>
