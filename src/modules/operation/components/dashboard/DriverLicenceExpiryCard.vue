<template>
  <div class="bg-surface rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col">
    <div class="flex items-center justify-between mb-4">
      <h3 class="font-semibold text-gray-800">Driver Licence Expiry</h3>
    ...
    <div v-else-if="!rows.length" class="text-sm text-gray-400 text-center py-8">
      No expiring licences in {{ days }} days
    </div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-100">
            <th class="text-left text-xs font-semibold text-gray-500 pb-2">Driver</th>
        ...
        <tbody>
          ...
          >
            <td class="py-2 text-xs text-gray-700">{{ row.name }}</td>
...
import { fetchDriverLicenceExpiresIn, fetchDriverLicenceExpired } from "../../api/dashboard.api";
...
const { data: expiresData, isLoading: loadingExpires } = useQuery({
  queryKey: ["dashboard-driver-licence-expires", days],
  queryFn: () => fetchDriverLicenceExpiresIn(days.value),
});

const { data: expiredData, isLoading: loadingExpired } = useQuery({
  queryKey: ["dashboard-driver-licence-expired"],
  queryFn: fetchDriverLicenceExpired,
});
...
const rows = computed(() => {
  ...
  return [...expired, ...expires]
    .filter((d: any) => { if (seen.has(d._id)) return false; seen.add(d._id); return true; })
    .map((d: any) => ({
      id: d._id,
      name: [d.firstName, d.middleName].filter(Boolean).join(" ") || "—",
      expiry: d.drivingLicenceExpirationDate
        ? new Date(d.drivingLicenceExpirationDate).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })
        : "—",
      expired: d.drivingLicenceExpirationDate ? new Date(d.drivingLicenceExpirationDate) < today : false,
    }));
});
</script>
