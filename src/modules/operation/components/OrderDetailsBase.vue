<template>
  <div class="h-full flex flex-col gap-4">
        ...
        <div class="flex-1 md:flex-none justify-end flex">
          <Dropdown v-if="order">
            <template #default="{ close }">
              <slot name="actions" :order="order" :close="close">
                <DropDownItem
    ...
    <div v-if="order" class="flex-1 min-h-0 overflow-y-auto">
      <slot :order="order" :active-tab="activeTab" :refetch="refetch">
        <component :is="activeTabComponent" :order="order" @refresh="refetch" />
      </slot>
    </div>
  </div>
</template>
...
// Tabs
import OrderOverviewTab from "./order-details/OrderOverviewTab.vue";
...
const { data: orderResponse, refetch } = useQuery({
  ...
  enabled: computed(() => !!orderId && orderId !== "add"),
});

const order = computed(() => orderResponse.value?.data as Order | undefined);
...
const handleOrderAction = (order: Order, action: string) => {
  if (action === "edit") {
    router.push(`${basePath.value}/edit/${order._id}`);
  } else if (action === "ship") {
    const shipPath = authStore.is_shipper
      ? "/shipper/shipments"
      : "/operation/shipments";
    router.push(`${shipPath}/add-from-order/${order._id}`);
  }
};
