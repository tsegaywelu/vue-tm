    ...
    <template v-else>
      <div class="flex justify-between items-center text-xs font-semibold text-gray-800">
        <span class="flex items-center gap-1.5">
          ...
          <span>Generating Payment Collection Excel</span>
        </span>
...
import { fetch_all_payment_collections_unpaginated } from "../../api/operation.api";
...
const statusText = ref("Fetching payment collections...");
let hasExported = false;

const closeToast = () => toastStore.removeToast(props.toastId);

const { data: response, isError, isFetching } = useQuery({
  queryKey: ["unpaginated_payment_collections", props.filters],
  queryFn: () =>
    fetch_all_payment_collections_unpaginated(props.filters ?? {}, {
...
function tryExport() {
  ...
  if (progress.value === 100 && rows) {
    ...
    XLSX.writeFile(wb, `PaymentCollections_${new Date().toISOString().split("T")[0]}.xlsx`);
...
watch(
  ...
  (err) => {
    if (err) {
      ...
      progress.value = 0;
      toastStore.error("Failed to download Payment Collections Excel.");
    }
  },
);
</script>
