  ...
  <div class="bg-surface rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col">
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-2">
        <span class="mdi mdi-file-alert-outline text-emerald-500 text-lg"></span>
        <h3 class="font-semibold text-gray-800">Delayed Invoices</h3>
      </div>
      <span class="text-xs font-semibold bg-emerald-50 text-emerald-600 px-2.5 py-1 rounded-full">
    ...
    <div v-else class="overflow-y-auto max-h-80 -mx-1 px-1 space-y-2">
      <RouterLink
        ...
        :key="alert.id || index"
        :to="`/finance/generateInvoice/${alert.paymentrequestId || ''}`"
      ...
      >
        <div class="flex items-start justify-between">
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-gray-400 ml-5">Approved {{ formatDate(alert.approvedTime) }}</p>
          </div>
...
const props = defineProps<{
  alerts: Array<{ id?: string; paymentrequestId?: string; message: string; approvedTime: string; isCritical: boolean }>;
  loading?: boolean;
}>();
