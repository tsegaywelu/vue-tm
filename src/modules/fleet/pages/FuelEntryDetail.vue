<template>
  <Teleport defer to="#page-actions">
    <template v-if="entry">
      <Button v-if="!entry.isVoided" variant="secondary" size="md" @click="router.push(`/fleet/fuel-entries/edit/${id}`)">
        Edit
      </Button>
  ...
  <div v-else-if="entry" class="flex flex-col gap-8">
    <InfoWrapper title="General Information">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-6 py-2">
        ...
        <ShipmentDataLabel label="Usage Type" :value="entry.usageType" />
    ...
    <InfoWrapper title="Fuel Details">
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 py-2">
        <ShipmentDataLabel label="Odometer Reading" :value="entry.odometer != null ? `${entry.odometer.toLocaleString()} km` : null" />
        <ShipmentDataLabel label="Quantity" :value="entry.quantity != null ? `${entry.quantity} L` : null" />
        <ShipmentDataLabel label="Unit Price" :value="entry.unitPrice != null ? currencyFormatter(entry.unitPrice) : null" />
        <ShipmentDataLabel label="Total Amount" :value="entry.amount != null ? currencyFormatter(entry.amount) : null" />
        <ShipmentDataLabel label="Vendor / Station" :value="entry.vendor" />
...
import { fetch_fuel_entry_by_id, void_fuel_entry, cancel_void_fuel_entry } from "../api/fleet.api";
...
const voidMutation = useMutation({
  mutationFn: ({ entryId, reason }: { entryId: string; reason: string }) =>
    void_fuel_entry(entryId, reason),
});

const cancelVoidMutation = useMutation({
  mutationFn: (entryId: string) => cancel_void_fuel_entry(entryId),
});
