...
const route_style = {
  true: 'rounded-[36px] text-white font-bold',
  false: 'rounded-3xl border-transparent hover:bg-grey-50',
  trueInner: 'rounded-[36px] bg-primary/5 text-primary font-bold',
  innerActive: 'text-primary font-semibold',
...
const is_active = computed(() => props.nav.path ? pathMatches(props.nav.path, route.path) : false)
const is_inner_active = computed(() => props.nav.children?.some(child => isRouteActive(child, route.path)))
const children_showable = computed(() => (props.nav.children || []).filter(child => child.show !== false))
  ...
  <div role="button" class="shadow-none flex flex-col gap-1 w-full">
    <button
      ...
      :class="[
        ...
        nested && is_active ? route_style.innerActive : 'text-grey-600',
        className
      ]"
    >
      <i
        ...
        :class="[
           ...
           is_active && !nested ? 'text-white' : is_active ? 'text-primary' : 'text-grey-400'
        ]"
        v-html="all_icons[nav.icon] || nav.icon"
      ></i>
      ...
      <span
        ...
        class="ml-auto size-5 grid place-items-center text-grey-300 transition-transform duration-300"
    ...
    <Transition
      ...
      @leave="(el: Element) => (el as HTMLElement).style.height = el.scrollHeight + 'px'"
